...
Write-Host "TubbyAI Working Deployment" -ForegroundColor Green
...
$TEMP_DIR = Join-Path $PROJECT_ROOT "working-deploy-$TIMESTAMP"
New-Item -ItemType Directory -Path $TEMP_DIR -Force | Out-Null

# Copy only the essential files
Write-Host "  Copying essential files..." -ForegroundColor Cyan
Copy-Item "$BACKEND_DIR\hello_world_fixed.py" "$TEMP_DIR\hello_world.py"
Copy-Item "$BACKEND_DIR\requirements_simple.txt" "$TEMP_DIR\requirements.txt"

# Create application.py for EB default Gunicorn
$APP_FILE = "from hello_world import app as application"
$APP_FILE | Out-File -FilePath (Join-Path $TEMP_DIR "application.py") -Encoding UTF8

# (Optional) Keep Procfile but not required
$PROCFILE_CONTENT = "web: gunicorn --bind 0.0.0.0:5000 --workers 1 --timeout 120 application:application"
$PROCFILE_CONTENT | Out-File -FilePath (Join-Path $TEMP_DIR "Procfile") -Encoding UTF8
...
RUN zip -r /tmp/tubby-working.zip . -x "*.git*" "*.pyc" "__pycache__/*"

# Copy ZIP to host
CMD ["cp", "/tmp/tubby-working.zip", "/output/"]
'@
...
try {
    docker build -t tubby-working:$TIMESTAMP .
    docker run --rm -v "${TEMP_DIR}/output:/output" tubby-working:$TIMESTAMP
    
    $ZIP_FILE = Get-ChildItem "output/tubby-working.zip" -ErrorAction SilentlyContinue
    if ($ZIP_FILE) {
        Write-Host "  ZIP created successfully" -ForegroundColor Green
        Copy-Item $ZIP_FILE.FullName (Join-Path $PROJECT_ROOT "tubby-working-$TIMESTAMP.zip")
    } else {
...
try {
    ...
    $ZIP_FILE_NAME = "tubby-working-$TIMESTAMP.zip"
    ...
    $VERSION_LABEL = "working-v-$TIMESTAMP"
...
Write-Host ""
Write-Host "Working deployment complete!" -ForegroundColor Green 