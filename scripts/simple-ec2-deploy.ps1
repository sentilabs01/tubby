# Simple EC2 Deployment for TubbyAI
param(
    [string]$InstanceType = "t2.micro"
)

Write-Host "TubbyAI Simple EC2 Deployment" -ForegroundColor Green
Write-Host "=============================" -ForegroundColor Green

# Configuration
$TIMESTAMP = Get-Date -Format "yyyyMMdd-HHmmss"
$KEY_NAME = "tubby-simple-key-$TIMESTAMP"
...
try {
    ...
    $sgName = "tubby-simple-sg-$TIMESTAMP"
...
'@
    ...
    # Launch instance
    $instanceId = aws ec2 run-instances --image-id ami-0c02fb55956c7d316 --count 1 --instance-type $InstanceType --key-name $KEY_NAME --security-group-ids $sgId --user-data $encodedUserData --region us-east-1 --query 'Instances[0].InstanceId' --output text
    ...
    $deploymentInfo | ConvertTo-Json | Out-File -FilePath "simple-ec2-deployment-$TIMESTAMP.json"

    Write-Host ""
    Write-Host "Simple EC2 deployment complete!" -ForegroundColor Green
    Write-Host "Deployment info saved to: simple-ec2-deployment-$TIMESTAMP.json" -ForegroundColor Cyan
